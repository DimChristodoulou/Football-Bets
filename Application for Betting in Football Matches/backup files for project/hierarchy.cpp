...
class Selection{
	...
	private:
		string name, name_app ,yield, yield_frac;
		bool visibility;
		float yield_dec;
	...
	public:
		...
		{
			string nam = name;
			name_app = nam.substr(0,nam.find("#"));
		}
...
class Market{
	...
	public:
		...
		void print_visible_selections(int toggle){	
			int j=0;
			...
			for(int i=0; i<sel_vector.size();i++){
				...
				sel_vector[i].set_name_app();
				
				if(sel_vector[i].is_visible()){
					if (toggle == 0)
					{
						cout << j+1 << ". " << sel_vector[i].get_name_app() << "#" << sel_vector[i].get_yield_fractional() << endl;
					}
					else if (toggle == 1)
					{
						cout << j+1 << ". " << sel_vector[i].get_name_app() << "#" << sel_vector[i].get_yield_decimal() << endl;
					}
					j++;
				}
			}
...
class Category{
	...
	public:
		...
		void print_visible_subcategories(){
			int j=0;
...
int update_hierarchy(ifstream &Read_hierarchy){
	...
	string a;
	while(getline(Read_hierarchy,line)){
		...
		std::size_t mydelim = line.find("\r");
		if (mydelim != string::npos)
			line = line.substr(0, mydelim);
		if(cat.size()==2){
			Category c(line);
